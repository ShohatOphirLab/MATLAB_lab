...
function [r_spearman,pvalueSpearman,Title,pmodel] = correlationDispMCBH(numberOfgroups,Data_vec_mean,color,name_behavior,h,padj)
   ...
   % plot(x_range, y_pred, "r");
    ...
    %     if abs(r_spearman) >= 0.3
            if lm.ModelFitVsNullModel.Pvalue <= 0.05
                ...
                saveas(gcf,'E:\hadar\GH\New folder\'+path_to_save)
            end
        end
    end
%end
