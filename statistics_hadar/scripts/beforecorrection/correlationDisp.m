function [r_spearman,pvalueSpearman,Title,pmodel] = correlationDisp(numberOfgroups,Data_vec_mean,color,name_behavior)
   ...
   % plot(x_range, y_pred, "r");
    ...
    title([Title, "pvalue = "+pvalueSpearman + ", R^2 = "+r_spearman])
    %pmodel = lm.ModelFitVsNullModel.Pvalue;
    ...
    %     if abs(r_spearman) >= 0.3
            if lm.ModelFitVsNullModel.Pvalue <= 0.05
                ...
                saveas(gcf,'F:\hadar\GH\New folder\'+path_to_save)
            end
        end
    end
%end
