function  correlationDispMCBHDH(numberOfgroups,Data_vec_mean,color,name_behavior,h,padj)
   ...
   % plot(x_range, y_pred, "r");
    ...
    xloca = [1 2 3 4];
    xticks(xloca);
    xlabels = {'Isolated','Bottle','RegularVial','SmallVial'};
    xticklabels(xlabels);
    %pmodel = lm.ModelFitVsNullModel.Pvalue;
    ...
    %     if abs(r_spearman) >= 0.3
           % if lm.ModelFitVsNullModel.Pvalue <= 0.05
                ...
                saveas(gcf,'D:\Hadar\DH\New folder\'+path_to_save)
            end
        end
    end
%end
