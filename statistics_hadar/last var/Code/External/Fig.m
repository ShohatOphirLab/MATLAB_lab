classdef Fig
    ...
    methods (Static = true)
        ...
        function CenterAndEqual()
            x = xlim;
            y = ylim;
            xlim(max(abs([x, y])) * [-1 1]);
            ylim(max(abs([x, y])) * [-1 1]);
        end
        ...
        function Text(first, varargin)
            ...
            switch opt.location
                case 'n'
                    ...
                    text(h, pos(1), pos(2), str, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', d{:}, aux{:});
                case 's'
                    pos = [(x(2)-x(1))/opt.margin,  y(1)+(y(2)-y(1))/opt.margin];
                    text(h, pos(1), pos(2), ['  ' str], 'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom', d{:}, aux{:});
                case 'w'
                    pos = [x(1)+(x(2)-x(1))/opt.margin,  -2*(y(2)-y(1))/opt.margin];
                    text(h, pos(1), pos(2), ['  ' str], 'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', d{:}, aux{:});
                case 'e'
                    pos = [x(2)-(x(2)-x(1))/opt.margin, -2*(y(2)-y(1))/opt.margin];
                    %pos = [x(2)-(x(2)-x(1))/opt.margin,  -(y(2)-y(1))/opt.margin];
                    text(h, pos(1), pos(2), ['  ' str], 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle', d{:}, aux{:});
                case 'nw'
                ...
                case 'ne'
                    ...
                    text(h, pos(1), pos(2), str, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', d{:}, aux{:});
                case 'se'
                    pos = [x(2)-(x(2)-x(1))/opt.margin, y(1)+(y(2)-y(1))/opt.margin];
                    text(h, pos(1), pos(2), str, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', d{:}, aux{:});
            end
        end
        ...
        function SavePDF(filename)
            ...
            if ~isMATLABReleaseOlderThan('R2020a')
                fn = fn.SetExt('.eps');
                exportgraphics(gcf,fn.Full,'ContentType','vector')
            else
            fn = fn.SetExt('.pdf');
            saveas(gcf, fn.Full);
            end
