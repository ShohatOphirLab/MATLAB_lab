classdef Patches
    ...
    methods (Static)
        function AnnotatedRect(str, x,y,w,h,c,varargin)
            Patches.Rect(x,y,w,h,c,varargin{:});
            text(x,min(y-h, y+h), ['  ', str] , 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
        end
        function AnnotatedCircle(str, x,y,r,c,varargin)
            Patches.Circle(x,y,r,c,varargin{:})
            text(x,y-r,  str , 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top');
        end
...
%             patch('Faces',1:length(x)+1,'Vertices',[x(:), y(:); [nan nan]], ds{:},'EdgeColor', c, varargin{:})
        ...
        function h = Rect(x,y,w,h,c,varargin)
            % Plot a rectangle on figure            
            if ~ishold
                cla reset
            end
            ds = Patches.DefaultStyle;
            h = patch([x x+w x+w x], [y y y+h y+h], c, ds{:}, varargin{:});
        end
        ...
        function LineOut(ax, x,y,width,c,varargin)
            %%
            ax.Units = "pixels";
            p = ax.InnerPosition;
            dx = ax.XLim(2) - ax.XLim(1);
            dy = ax.YLim(2) - ax.YLim(1);
            nx = ((x - ax.XLim(1)) / dx) * p(3) + p(1);
            ny = ((y - ax.YLim(1)) / dy) * p(4) + p(2);
            annotation("line", nx, ny, 'units', 'pixels', 'LineWidth', width, 'Color', c);
        end
