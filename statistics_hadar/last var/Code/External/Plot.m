classdef Plot
    methods (Static = true)
        ...
        function stat = Corr(x,y,varargin)
            %%
            plot (x, y, varargin{:});
            l = fitlm(x, y);
            stat.pval = l.Coefficients.pValue(1);
            hold on
            x_ = xlim;
            plot(x_, l.predict(x_(:)), 'k');
            [stat.corr, stat.corr_pval] = Q.nancorr(x, y);
            stat.r_square = l.Rsquared.Ordinary;
            hold off;
            Fig.TextSW({...
                sprintf('\\rho=%.2g', stat.corr), ...
                [sprintf('p=%.2g', stat.corr_pval), ...
                sprintf(', r^2=%.2g', stat.r_square)]});
        end
...
%             set(gca, 'YTick', 1:min(diff(get(gca, 'YTick'))):max(get(gca, 'YLim')));
        ...
        function P = Statistics(data, varargin)
            ...
            w = .4;
            %
            cmap = [0.75 0.84 1;0.47 0.65 1;0.24 0.49 1;0 0.22 1;0 0.17 0.52];%[0.75 0.84 1;0 0.22 1;0 0.17 0.52;0.47 0.65 1;0.24 0.49 1];%opt.colormap;
...
%                 range(2) = ceil(m2 / scale) * scale;
            ...
            for y=1:size(mat, 1)
                for x=1:size(mat, 2)
                    ...
                    if istext & valid(y, x)  
                        Shapes.Text(x, y, sprintf('%.1f', mat(y, x)), 'k');
                    end
                    Fig.Hon
                end
            end
