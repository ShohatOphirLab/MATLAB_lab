...
classdef Colors
    ...
    methods (Static)
        ...
        function c = Darken(c, beta)
            % Make color darker. Beta larger than 0
            %             hsv = rgb2hsv(reshape(c, [1 1 3]));
            %             hsv(2:3) = 1 - (1 - hsv(2:3)) * (1 - beta);
        ...
        function color = fromString(str)
            ...
            if isempty(str)
                color = [0, 0, 0];
            elseif length(str) == 7 && str(1) == '#'
                color = Colors.FromHex(str(2:end));
            elseif length(str) == 6 && all(ismember(str, '0123456789ABCDEF'))
                color = Colors.FromHex(str);
            elseif length(str) == 1

                switch lower(str)
                    case 'r'
                        color = Colors.PrettyRed;
                    case 'g'
                        color = Colors.PrettyGreen;
                    case 'b'
                        color = Colors.PrettyBlue;
                    case 'c'
                        color = Colors.FromHex('00FFFF');
                    case 'm'
                        color = Colors.FromHex('FF00FF');
                    case 'y'
                        color = Colors.Yellow;
                    case 'k'
                        color = Colors.Black;
