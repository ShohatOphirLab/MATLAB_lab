...
% load('../Data/GroupedHouse/cellBehavior_GH.mat');
...
for i = 1:length(remove)
    map = fullTable.fly == remove(i).fly & fullTable.movie_number == remove(i).movie_number;
    fullTable(map, :) = [];
end
for i=1:size(fullTable.condition, 1); curr=fullTable.condition{i}; curr = sprintf('%02d', str2double(regexprep(curr, 'GroupedHouse', '')));fullTable.condition{i}=curr;  end

warning('ensure the following two lines are needed (and not doing damage)');
fullTable.Properties.VariableNames{6} = 'scores_Chain_hadar';
fullTable.Properties.VariableNames{7} = 'scores_Chase_hadar';
