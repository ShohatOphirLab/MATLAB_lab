 ...
 # order_name<-as.data.frame(read_excel("D:/MATLAB/flybowl_general/forscatter/order.xlsx"))
  ...
  if(type_format==3){
   ...
   # Cairo(800,400,file=paste(paths,"/", ".svg", sep=""),type="svg",bg="transparent",height=height, width=width,units = "in",res=400)
    ggsave(file=paste(paths,"/", ".svg", sep=""), plot=t, width=width, height=height)
    plot(t)
    dev.off()
...
debbug_path_color<-"W:/Julia/for the aricle after new analysis/NPFrNPFactivation/color.xlsx"
debbug_path_param<-"W:/Julia/for the aricle after new analysis/NPFrNPFactivation/params.xlsx"
debbug_path_border<-"W:/Julia/for the aricle after new analysis/NPFrNPFactivation/border_color.xlsx"
