...
for i = 1:length(handles.allFolders)
    ...
    for j =1:length(genotypeData)
        ...
        arr(j)=mean(mean(current_arena(7200:9000,:)))
    end
    %David
    

       outputNameAllTable = fullfile(path, file(1:k(end)-1));
    % mkdir(outputNameAllTable) %CAN CAUSE ERROR IF ALREADY exiss
    outputNameAllTable = [outputNameAllTable, ' per_arena.csv'];
    writematrix(arr, outputNameAllTable);

    outputNameAllTable = fullfile(path, file(1:k(end)-1));
    % mkdir(outputNameAllTable) %CAN CAUSE ERROR IF ALREADY exiss
    outputNameAllTable = [outputNameAllTable, ' all.csv'];
    writematrix(all, outputNameAllTable);
