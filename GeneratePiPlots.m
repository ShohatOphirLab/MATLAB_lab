...
% Initialize lastMinAvg as NaN matrix with dynamic rows and columns
lastMinAvg = NaN(length(handles.allFolders), max(cellfun(@numel, handles.allFolders)));
...
for i = 1:length(handles.allFolders)
    ...
    lastFrame = min(nrows);
    % numFramesPerMin = 60 * handles.framePerSec;     % Calculated by timestamp when possible.Sometimes the calculation yields a decimal number, so a magic number is used.
    numFramesPerMin = 60 * 30; % hard-coded version
    firstLastMinFrame = lastFrame - numFramesPerMin;

    arr = [];
    for j = 1:length(genotypeData)
        current_arena = genotypeData{j};
        arr(j) = mean(mean(current_arena(firstLastMinFrame:lastFrame,:)));
    end
    lastMinAvg(i, 1:numel(arr)) = arr;

    % Define base file path and genotype directory name
    baseFileName = fullfile(path, file(1:k(end)-1));
    genotypeDirName = [handles.genotypesNames{i}, '_PiData'];
    
    % Generate initial path
    genotypePath = fullfile(baseFileName, genotypeDirName);
    
    % Initialize counter for unique folder creation
    counter = 1;
    newGenotypePath = genotypePath;
    
    % Check if folder exists and create a unique folder name
    while isfolder(newGenotypePath)
        newGenotypePath = fullfile(baseFileName, [genotypeDirName, ' (', num2str(counter), ')']);
        counter = counter + 1;
    end
    
    % Create the directory
    try
        mkdir(newGenotypePath);
        fprintf('Directory created: %s\n', newGenotypePath);
    catch ME
        warning('%s', ['Failed to create directory: ', ME.message]);
        error('Cannot proceed without a valid directory.');
    end


    fullGenotypeDataFileName = fullfile(genotypePath, [handles.genotypesNames{i}, '_PiData.xlsx']);
    arenaAvgFileName = fullfile(genotypePath, [handles.genotypesNames{i}, '_arenaAvg.xlsx']);

    arenaAvgArr = zeros(length(genotypeData), min(nrows));

    for j = 1:length(genotypeData)
        current_arena = genotypeData{j}; % Original: numFrames x numFlies
        switched_arena = current_arena'; % Now: numFlies x numFrames

        % Create row and column names for full data table       
        rowNamesForData = arrayfun(@(x) sprintf('FlyNum%d', x), 1:size(switched_arena, 1), 'UniformOutput', false);
        colNamesForData = arrayfun(@num2str, 1:size(switched_arena, 2), 'UniformOutput', false);

        % Convert the transposed arena to a table
        arenaTable = array2table(switched_arena, 'RowNames', rowNamesForData, 'VariableNames', colNamesForData);
        
        % Save the data to a new sheet in the Excel file
        sheetName = sprintf('Arena%d', j);
        writetable(arenaTable, fullGenotypeDataFileName, 'Sheet', sheetName, 'WriteRowNames', true);

        % Calculate average for each frame (column) in switched_arena
        frameAverages = mean(switched_arena, 1);   
        arenaAvgArr(j, :) = frameAverages;  % Store the averages in arenaAvgMat
    end
    
    % Create row and column names for avgPerFrame table
    % colNamesForAvg = arrayfun(@(x) sprintf('Frm%d', x), 1:nrows, 'UniformOutput', false);
    % rowNamesForAvg = arrayfun(@(x) sprintf('Arena%d', x), 1:length(genotypeData), 'UniformOutput', false);
    
    % Convert arenaAvgMat to a table with row and column names
    % arenaAvgTable = array2table(arenaAvgArr, 'RowNames', rowNamesForAvg, 'VariableNames', colNamesForAvg);
    arenaAvgTable = array2table(arenaAvgArr);
    
    % Write the arena averages to the arenaAvg.xlsx file, including row names
    % writetable(arenaAvgTable, arenaAvgFileName, 'Sheet', 'arenaPerFrame','WriteRowNames', true, 'WriteVariableNames', true);
    writetable(arenaAvgTable, arenaAvgFileName, 'Sheet', 'arenaPerFrame','WriteRowNames', false, 'WriteVariableNames', false);
...
% Ensure rowTitles is a column vector
rowTitles = handles.genotypesNames(:); % Convert to column vector if it's a row vector
% Add column titles for the arenas
colTitles = arrayfun(@(x) sprintf('arena_%d', x), 1:size(lastMinAvg, 2), 'UniformOutput', false);

% Create header row and combine everything into a cell array
headerRow = ['GenotypeName', colTitles]; % First column name is "GenotypeName"
lastMinAvgCell = [headerRow; [rowTitles, num2cell(lastMinAvg)]];

for i = 1:length(handles.allFolders)
    genotypeDirName = [handles.genotypesNames{i}, '_PiData'];
    genotypePath = fullfile(baseFileName, genotypeDirName);
    arenaAvgFileName = fullfile(genotypePath, [handles.genotypesNames{i}, '_arenaAvg.xlsx']);
    writecell(lastMinAvgCell, arenaAvgFileName, 'Sheet', 'lastMinAvg');
end
